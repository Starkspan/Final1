...
--------------------------
Material: ${material}
Maße: ${x1} x ${x2} x ${x3} mm
Gewicht: ${gewicht.toFixed(2)} kg
Laufzeit: ${laufzeit_min.toFixed(1)} Minuten
Materialkosten: ${materialkosten.toFixed(2)} €
Endpreis/Stück: ${einzelpreis_final.toFixed(2)} €
Zielpreis: ${zielpreis || "-"}
Stückzahl: ${stueckzahl}
    `;

    await transporter.sendMail({
      from: '"Starkspan Rechner" <info@starkspan.com>',
      to: 'info@starkspan.com',
      subject: 'Neue CNC-Anfrage von WebApp',
      text: mailtext
    });
    ...
    console.error(err);
    res.status(500).json({ error: 'Fehler bei Google OCR oder Mailversand' });
  }
});
